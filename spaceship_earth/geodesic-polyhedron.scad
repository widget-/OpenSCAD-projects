...
This script was initially generated by
http://kitwallace.co.uk/3d/solid-index.xq?mode=solid&id=Icosahedron

It was slightly modified by Bryan Keller to get the correct shape for
the modified geodesic polyhedron that Spaceship Earth uses.

Spaceship Earth is a kdcccdI geodesic polyhedron. between the "k" and
"d" steps the vertices need to be normalized such that they are
equidistant from the center to give it the "ball" shape. Then the final
kis step can apply the iconic trianglular face pattern.

Check https://en.wikipedia.org/wiki/Geodesic_polyhedron for more

The main scad files use a pre-rendered STL version of this. On my
moderately powerful computer, it can still take up to a minute to render
on its own.

*/

// source=conway form=solid id =Icosahedron
// formula kdcccdI
solid_1 = plane(dual(chamfer(chamfer(chamfer(dual(I()))))),10);
p_describe(solid_1);
function fmod(r,theta,phi) = [1, theta,phi];
solid_2 = modulate(solid_1);
solid_3 = kis(solid_2, h=-0.03);
show_solid(solid_3);
